...
class SeedTestDataTest {
    ...
    private var loadAccounts:
        (AccountTypeManager.AccountFilter?) -> Flow<PersistentList<AccountDisplayModel>> =
        { emptyFlow() }
    ...
    @Before
    fun setUp() {
        SeedTestDataImpl.contactsCount = 3
    }

    @Test
    fun usesDeviceAccount_ifAvailable() = runTest {
        val deviceAccount = AccountDisplayModelFactory.build(isDeviceAccount = true)
        val otherAccount = AccountDisplayModelFactory.build(isDeviceAccount = false)
        loadAccounts = { flowOf(persistentListOf(otherAccount, deviceAccount)) }
        every { getDefaultAccount() } returns otherAccount.account

        buildSubject()()

        verify(exactly = SeedTestDataImpl.contactsCount) {
    ...
    fun usesDefaultAccount_ifThereIsNoDeviceAccount() = runTest {
        val defaultAccount = AccountDisplayModelFactory.build(isDeviceAccount = false)
        val otherAccount = AccountDisplayModelFactory.build(isDeviceAccount = false)
    ...
    @Test
    fun usesDefaultAccount_ifThereIsNoDeviceOrDefaultAccount() = runTest {
        val firstAccount = AccountDisplayModelFactory.build(isDeviceAccount = false)
        val secondAccount = AccountDisplayModelFactory.build(isDeviceAccount = false)
    ...
    fun setsPhoneValuesCorrectly() = runTest {
        SeedTestDataImpl.contactsCount = 1
        loadAccounts = { flowOf(persistentListOf(AccountDisplayModelFactory.build())) }
    ...
    private fun buildSubject(): SeedTestData {
