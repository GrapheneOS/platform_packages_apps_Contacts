...
class SdnProvider : ContentProvider() {
    ...
    override fun onCreate(): Boolean {
        Log.i(TAG, "onCreate")
        val sdnProviderAuthority = requireContext().getString(
            R.string.contacts_sdn_provider_authority
        )
    ...
    private fun handleFilter(projection: Array<out String>, uri: Uri): Cursor? {
        ...
        val results =
            sdnRepository.fetchSdn().filter {
                it.serviceName.contains(filter, ignoreCase = true) ||
                    it.serviceNumber.contains(filter)
            }
    ...
    private fun handlePhoneLookup(
        ...
        val result =
            sdnRepository.fetchSdn().find { it.serviceNumber == phoneNumber } ?: return cursor
