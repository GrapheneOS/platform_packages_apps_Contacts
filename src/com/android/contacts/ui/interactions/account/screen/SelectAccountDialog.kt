...
private fun SelectAccountDialogListPreview() {
    val account1 = AccountModel(name = "user1@example.org")
    val accountUi1 = AccountUiModel(account = account1, name = account1.name, type = "Google")
    val account2 = AccountModel(name = null)
    val accountUi2 = AccountUiModel(account = account2, name = null, type = "Device")
    ContactsPreviewTheme {
        Box(Modifier.fillMaxSize()) {
            SelectAccountDialogContent(
                uiState = State(accounts = persistentListOf(accountUi1, accountUi2)),
                onAction = {},
            )
        }
    }
